...
class Wlan(Common):
    ...
    def pull(self, body, host, org_id):
        ...
        if "site_id" in body:
            ...
            return self._get_wlans(body, "sites", "site_id", host, org_id)
        else:
            logging.debug("new org_id request")
            return self._get_wlans(body, "orgs", "org_id", host, org_id)
        # else:
        #     logging.error(
        #         "new request without site_id not org_id: {0}".format(body))
        #     return {"status": 500, "data": {"message": "site_id or org_id missing"}}

    def _get_wlans(self, body, scope_name, scope_id_param, host, org_id):
        if scope_name == "sites":
    ...
    def _find_wlans(self, extract, ssid, scope_name, scope_id, host, org_id):
        url = "https://{0}/api/v1/{1}/{2}/wlans".format(
            host, scope_name, scope_id)
        logging.debug("REQ: {0}".format(url))
        resp = requests.get(
            url, headers=extract["headers"], cookies=extract["cookies"])
        logging.debug("REQ: OK")
    ...
    def _get_wlan_by_id(self, extract, wlan_id, scope_name, scope_id, host, org_id):
        url = "https://{0}/api/v1/{1}/{2}/wlans/{3}".format(
            host, scope_name, scope_id, wlan_id)
        logging.debug("REQ: {0}".format(url))
        resp = requests.get(
            url, headers=extract["headers"], cookies=extract["cookies"])
        logging.debug("REQ: OK")
    ...
    def change_vlan(self, body, host, org_id):
            ...
            for check in vlan_check:
                if check["code"] in ["untagged", "vlan_pooling_disabled"]:
                    ...
                    wlan_conf = self._get_wlan_by_id(
                        extract, check["wlan_id"], check["scope_name"], check["scope_id"], host, org_id)
                    wlan_conf = {
                        "vlan_enabled": True,
                        "vlan_id": None,
                        "dynamic_vlan": None,
                ...
                try:
                    url = "https://{0}/api/v1/{1}/{2}/wlans/{3}".format(
                        host, check["scope_name"], check["scope_id"], check["wlan_id"])
                    logging.debug("REQ: {0}".format(url))
                    resp = requests.put(
